...
\begin{frame}{Practical session 1: Manipulating variables and loading data}
	Referring to the exercise sheet:
	\begin{enumerate}
		\item At the prompt gain familiarity with creating variables, and mathematical operations
		\item Practice loading and saving data
		\item Import data from csv file and Excel spreadsheet
	\end{enumerate}
\end{frame}
...
\frame{\frametitle{Toolboxes}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Enter \texttt{ver} to see list of installed toolboxes
		\end{itemize}
...
\begin{frame}{Practical session 2: Functions and plotting}
	\begin{enumerate}
		\item Use MATLAB's help to find functions
		\item Use \texttt{plot} function to create 2D line plot
	\end{enumerate}
\end{frame}
...
\frame{\frametitle{Floating point rounding}
	\begin{itemize}
		...
		\item For example \lstinline[style=Matlab-editor]!eps! returns the distance between 1.0 and
		the next largest double-precision number
		\item  Real numbers between 1.0 and 1.0+eps can't be represented
		using MATLAB doubles
		\item Values between 1.0 and 1.0 + eps will be rounded
...
ans = 
char
		\end{lstlisting} 	% These are not typos above - results are not indented.
